...
def class UniqueMerge[A](is :: List[Getter[A]], o :: Putter[A]) = 
  ...
  def fillTops() =
    joinMap(lambda(i :: Integer) = (if( tops(i)? = null ) then 
    		  tops(i) := index(is, i)()
    		else 
    		  signal), listUpto(length(is)))
  def getMin() =
    fillTops() >> (
    val (mi, mv, _) = foldl(lambda((mi, mv, i) :: (Integer, A, Integer), v :: A) = 
    								if (if mv /= null then v >= mv else false) then (mi, mv, i+1) else (i, v, i+1), 
    						(-1,Null[A](),0), arrayToList(tops))
    --Println("Filled" + arrayToList(tops)) >> 
    joinMap(lambda(i :: Integer) = (if( tops(i)? = mv ) then 
    		  tops(i) := Null[A]()
    		else
    		  signal), listUpto(length(is))) >>
    mv)
...
Trans(lambda(x :: Integer) = x*2, x2.get, x2'.put) >> stop | 
