//
// OrcSite.scala -- Scala class/trait/object OrcSite
// Project OrcScala
//
// $Id$
//
...
object OrcSiteCallTarget {
  ...
  private val targets = scala.collection.mutable.Set[OrcSiteCallTarget]()
  ...
  /** Collect all collectable OrcSites. Halting their declarations.
    ...
    * 
    * For this to catch unused OrcSites the JVM GC must run occationally. This can
    * be triggered using System.gc().
    */
  def collect(): Unit = {
    for (ref <- Iterator.continually(queue.poll()).takeWhile(_ != null)) {
      ref match {
  ...
  def create(s: OrcSite) = synchronized {
    targets.add(new OrcSiteCallTarget(s))
  }
}
