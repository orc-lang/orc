...
object DistribScaleTestHtCondor {
  ...
  def buildCondorSubmissions(testRunNumber: String, experimentalConditions: Traversable[DistribScaleExperimentalCondition], testPrograms: Traversable[File]): Traversable[OrcCondorJobCluster] = {
    val jobClusters = ArrayBuffer[OrcCondorJobCluster]()
  ...
  protected class OrcCondorJobCluster(testRunNumber: String, testProgram: File, experimentalCondition: DistribScaleTestCase.DistribScaleExperimentalCondition) {
  ...
  protected def runCondorSubmissions(testRunNumber: String, condorSubmissions: Traversable[OrcCondorJobCluster]) = {
    ...
    for (condorSubmission <- condorSubmissions) {
      ...
      try {
        subFileWriter.append(condorSubmission.submitDescription)
      } finally {
        subFileWriter.close()
      }
      val remoteRunOutputDir = DistribTestConfig.expanded("runOutputDir").stripSuffix("/")
    ...
    println(s"Orc run $testRunNumber HTCondor submission complete.")
  }
