...
import java.awt.event.{ KeyEvent, KeyListener, MouseWheelEvent }
import java.awt.geom.{ Ellipse2D, Line2D, Point2D, Rectangle2D }
import java.nio.file.{ Files, Paths }
import java.time.Duration

import scala.collection.JavaConverters.asScalaIteratorConverter
import scala.collection.immutable.{ LongMap, SortedMap }

import javax.swing.{ JFrame, JPanel, JScrollPane, ScrollPaneConstants, SwingUtilities, WindowConstants }
...
object TraceTimelineViewer {
  ...
  def loadTimeline(inputPathname: String): Timeline = {
    val reader = Files.newBufferedReader(Paths.get(inputPathname))
  ...
  def main(args: Array[String]): Unit = {
    val Array(inputPathname) = args
    val timeline = loadTimeline(inputPathname)
