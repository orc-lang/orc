...
def tabulateArrayDouble(n, f) =
	val a = Array(n, "double")
	upto(n) >i> a(i) := f(i) >> stop ;
	a

def tabulateArrayDoubleSeq(n, f) =
    val a = Array(n, "double")
    sfor(0, n, lambda(i) = a(i) := f(i)) >>
    a

def tabulateArrayObject(n, f) =
	val a = Array(n)
	upto(n) >i> a(i) := f(i) >> stop ;
	a

def mapArrayDouble(a, f) =
	val n = a.length?
...
-- Lines: 11
def simPathForward(swaption :: Swaption, forwards :: Array[Double], totalDrift :: Array[Double], seed :: Integer) :: Array[Array[Double]] =
...
-- Lines: 7
def discountFactors(nSteps :: Integer, years :: Double, path :: Array[Double]) :: Array[Double] =
...
-- Lines: 27
def simulate(swaption, seed) :: Number = (
    val timeDelta = swaption.years() / nSteps
