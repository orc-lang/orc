...
-- Lines: 4
def sha1(chunk) = ArrayKey( -- Inferable (type)
...
def readSegements(minimumSegmentSize, in) =
	def process(currentChunk, i) = (
		...
		if splitPoint = currentChunk.size() then
			...
			-- Inferable (multiple publications)
...
def segment(minimumSegmentSize, chunk) =
	...
	def process(chunk, i) = ( -- Inferable (multiple publications)
...
-- Lines: 8
def writeChunk(out, cchunk, isAlreadyOutput) = ( -- Inferable
...
def write(out, outputPool) =
	...
	def process((roughID, fineID), id) = (
		...
		else if cchunk.uncompressedSize = 0 then ( -- Inferable
