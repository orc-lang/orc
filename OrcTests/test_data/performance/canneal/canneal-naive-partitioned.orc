...
def run(netlist) =
    ...
    def partition(temperature, n) = Sequentialize() >> Ift(n :> 0) >>  (
        ...
        ) >> stop ; partition(temperature, n - 1)
    ...
    def anneal(temperature, tempStep) =
    	upto(nThreads) >> Sequentialize() >> partition(temperature, nSwapsPerThread) >> stop ; (
...
benchmarkSized("Canneal-naive-partitioned", nTempSteps * swapsPerTemp, { netlist }, run)
