...
def sforBy(low, high, step) = Sequentialize() >> (
  Iff(low >= high) >> (low | sforBy(low+step, high, step))
  )
...
def run(data) =
	...
	riskless >> volatility >> res >> partitionSize >>
...
benchmarkSized("Black-Scholes-scala-compute-partitioned-seq-optimized", data.length?, { data }, run, BlackScholesData.check)
