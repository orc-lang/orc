...
/**
 * This class was factored out of ElizaMain for use in Orc,
 * by Adrian Quark.
 */
public class Eliza {
    private final boolean echoInput = false;

    private final boolean printKeys = true;
    private final boolean printSyns = true;
    private final boolean printPrePost = true;
    private final boolean printInitialFinal = true;
    ...
    public void collect(String s) throws IOException {
        ...
        if (EString.match(s, "*reasmb: *", lines)) {
            if (lastReasemb == null) {
            	throw new IOException("Error: no last reassemb");
            }
            lastReasemb.add(lines[1]);
        }
        else if (EString.match(s, "*decomp: *", lines)) {
            if (lastDecomp == null) {
            	throw new IOException("Error: no last decomp");
            }
            ...
            if (lines[2].length() != 0) {
                try {
                	...
                	throw new IOException("Number is wrong in key: " + lines[2]);
                }
            }
        	...
        	throw new IOException("Unrecognized input: " + s);
        }
    }
     ...
     * @throws IOException 
     */
    public String processInput(String s) throws IOException {
        String reply;
     ...
     * @throws IOException 
     */
    private String sentence(String s) throws IOException {
        s = pre.translate(s);
        s = EString.pad(s);
        if (quit.find(s)) {
            finished = true;
     ...
     * @throws IOException 
     */
    private String decompose(Key key, String s, Key gotoKey) throws IOException {
     ...
     * @throws IOException 
     */
    private String assemble(Decomp d, String reply[], Key gotoKey) throws IOException {
        ...
        if (EString.match(rule, "goto *", lines)) {
            ...
            if (gotoKey.key() != null) return null;
            throw new IOException("Goto rule did not match key: " + lines[0]);
        }
        String work = "";
        while (EString.match(rule, "* (#)*", lines)) {
            ...
            try {
            	...
            	throw new IOException("Number is wrong in reassembly rule " + lines[1]);
            }
            if (n < 0 || n >= reply.length) {
                throw new IOException("Substitution number is bad " + lines[1]);
            }
    ...
    public void readScript(Reader in) throws IOException {
