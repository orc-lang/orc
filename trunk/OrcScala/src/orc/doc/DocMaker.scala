...
object DocMaker {
  ...
  def renderSection(f: File) = {
    val docItemList = List(DocText("")) // FIXME ... DocParsers.parse(f)
    val optionalHeader = {
      docItemList match {
  ...
  def renderItems(items: List[DocItem])(implicit nextCode: String): List[Node] = {
    items match {
      case DocText(s) :: rest => {
        val paraNodes = paragraphs(s) map { p: String => <para>{ p }</para> }
      ...
      case (_ : DocDecl) :: _ => {
        ...
        val declChunk = {
          <variablelist>
            ...
            { decls map { d: DocItem => renderDecl(d.asInstanceOf[DocDecl]) } }
          </variablelist>  
        }
