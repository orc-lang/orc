...
// Project OrcScala
//
// $Id: ExecutionExceptions.scala 2262 2010-12-16 14:55:25Z jthywissen $
//
 ...
 * 
 */
class StackLimitReachedError(val limit: Int) extends
 TokenException("Stack limit (limit=" + limit + ") reached")

/**
 * 
 */
class TokenLimitReachedError(val limit: Int) extends
 TokenError("Token limit (limit=" + limit + ") reached")

/**
 * 
 */
class UncaughtException(message: String, cause: Throwable) extends
 ...
 */
abstract class RuntimeTypeException(message: String) extends
 TokenException(message)

/**
 * per JLS section 15.12.2.5
 */
class AmbiguousInvocationException(val methodNames: Array[String]) extends
 RuntimeTypeException("Ambiguous method invocation: "+methodNames.mkString("  -OR-  "))

/**
 * 
 */
class ArgumentTypeMismatchException(val argPosition: Int, val expectedType: String, val providedType: String) extends
 RuntimeTypeException("Expected type " + expectedType + " for argument " + argPosition + ", got " + providedType + " instead")

/**
 * 
 */
class ArityMismatchException(val arityExpected: Int, val arityProvided: Int) extends
 RuntimeTypeException("Arity mismatch, expected " + arityExpected + " arguments, got " + arityProvided + " arguments.")

/**
 * 
 */
class InsufficientArgsException(val missingArg: Int, val arityProvided: Int) extends
 RuntimeTypeException("Arity mismatch, could not find argument " + missingArg + ", only got " + arityProvided + " arguments.")
 ...
 * 
 */
class MethodTypeMismatchException(val methodName: String, val clazz: Class[_]) extends
 RuntimeTypeException("Argument types did not match any implementation for method '" + methodName + "' in " + clazz.getName() + ".")
 ...
 */
class JavaException(cause: Throwable) extends
 SiteException(cause.toString(), cause) {
  /**
