...
import orc.run.Orc//.{Execution, Group, Token}
...
object ProgSwap extends Site with UntypedSite {
  ...
  override def call(args: List[AnyRef], callHandle: Handle) {
    def tokenCracker(token: Handle): Orc#Token = token.asInstanceOf[Orc#Token]
    val execGroup: Orc#Execution = tokenCracker(callHandle).group.root.asInstanceOf[Orc#Execution]
...
//    }
   ...
   */
  def update(execGroup: Orc#Execution, newOilFile: File): Boolean = {
   ...
   * @throws CompilationException If new OIL file fails to unmarshal or resolve 
   */
  protected def loadNewProgram(newOilFile: File, execGroup: Orc#Execution): NamelessAST = {
    Console.err.println(">>Load new OIL file "+newOilFile)
   ...
   */
  protected def suspendEngine(execGroup: Orc#Execution) {
    execGroup.suspend()
   ...
   * @return
   */
  protected def isSafeState(execGroup: Orc#Execution, oldOilAst: NamelessAST, newOilAst: NamelessAST, editList: AstEditScript): Boolean = {
   ...
   * @param editList
   */
  protected def migrateTokens(execGroup: Orc#Execution, oldOilAst: NamelessAST, newOilAst: NamelessAST, editList: AstEditScript) {
   ...
   */
  protected def updateConfig(execGroup: Orc#Execution, newOilFileName: String) {
   ...
   */
  protected def resumeEngine(execGroup: Orc#Execution) {
    execGroup.resume()
  }
