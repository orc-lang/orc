...
public class Token implements Serializable, Locatable {
	/**
	 * Return pointer for a function call. At one point we used a token for the
	 * return pointer, but that was really an abuse of tokens.
	 * 
	...
	/*
	 * Exception frame. This also must record enough data to create a new Token
	 * in the context of the handler (its sort of like a Continuation but we
	 * need more data like the region and the group etc.)
	 */
	...
	/**
	 * Before doing anything, the token checks if its group is alive. If not, it
	 * kills itself. This is how forced termination is implemented.
	 */
	private Group group;
	/**
	 * A region corresponds to a dynamic scope in a running program. Tokens can
	 * enter and leave regions. When all the tokens have left a region, it is
	 * closed. This is how termination is detected.
	 */
	private Region region;
	/** The engine executing the program. */
	private OrcEngine engine;
	/**
	 * The location of the token in the source code. This is set whenever the
	 * token begins processing a new node. Why not just use the location of the
	 * current node? Because e.g. during a site call this.node actually points
	 * to the next node, not the current one, so the source location would be
	 * incorrect.
	 */
	...
	/**
	 * The continuation determines where to return when a token reaches the end
	 * of a function call.
	 */
	...
	/**
	 * Each token has an associated logical clock. The token notifies the clock
	 * when it becomes quiescent. When all the clock's tokens are quiescent, it
	 * can advance logical time.
	 */
	...
	/**
	 * Used when throwing a Java exception. Note this also stores source
	 * location for java exceptions
	 */
	...
	/**
	 * What kind of exception are we handling? Used for handling errors
	 * correctly
	 */
	...
	/**
	 * Kill this token. Should only be called on non-quiescent tokens.
	...
	/**
	 * Enter a closure by moving to a new node and environment, and setting the
	 * continuation for {@link #leaveClosure()}.
	 * 
	...
	public final Token enterClosure(final Closure closure, final TokenContinuation publishContinuation) throws StackLimitReachedError {
		...
		if (node instanceof Call && ((Call) node).isTailCall) {
			...
			// Push a frame, so we can return to this environment after exiting
			// the closure
			continuation = new FrameContinuation(publishContinuation, this.env.clone(), continuation, location);
		}

		/*
		 * If the stack limit is on (stackAvailable >= 0), make sure we haven't
		 * exceeded it.
		 */
	...
	/*
	 * Called when an exception is thrown. Kill the current token, and create a
	 * new token in the correct group/region to call the handler
	 */
	public void throwException(final Object exceptionValue) throws TokenLimitReachedError, TokenException {
		...
		/*
		 * If this is the first time we've seen this exception (ie, we haven't
		 * done a re-throw), record error reporting information.
		 */
		...
		if (exceptionStack.empty()) {
			/*
			 * We have an uncaught exception, and the backtrace should point to
	...
	public void pushHandler(final Closure closure, final TokenContinuation handlerReturn) {
		...
		/*
		 * Create the continuation Token, but don't run it by removing it from
		 * the region
		 */
