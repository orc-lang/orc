 ...
 * Contributors: 	
 *		Martin D. Kirk
 * ORC: added support for <, >, and & auto-complete.
 *
	...
	snippets : function(evt) {
		...
		for (var i=0; i<snippets.length; i++) {
			if(snippets[i].input == trigger) {
				var content = this.escapeHTML(snippets[i].output);
				if(content.indexOf('$0')<0) content += cc;
				else content = content.replace(/\$0/,cc);
				var pattern = new RegExp(this.escapeHTML(trigger)+cc,'gi');
	...
	complete : function(trigger) {
		...
		for (var i=0; i<complete.length; i++) {
			if(complete[i].input == trigger) {
				var pattern = new RegExp('\\'+this.escapeHTML(trigger)+cc);
				var content = this.escapeHTML(complete[i].output);
				if(content.indexOf('$0')<0) content += cc;
				else content = content.replace(/\$0/,cc);
