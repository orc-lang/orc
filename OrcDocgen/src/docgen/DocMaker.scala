...
import scala.xml.{ Comment, Elem, Node, PCData, PrefixedAttribute, Unparsed }
...
class DocMaker(toplevelName: String) {
  ...
  def renderChapter(targets: Iterable[Path]): Node = {
    <chapter xml:id={ toplevelName } xmlns="http://docbook.org/ns/docbook" xmlns:od="http://orc.csres.utexas.edu/OrcDocBook.xsd" xmlns:xi="http://www.w3.org/2001/XInclude">
      ...
      { targets map { t => <xi:include href={ t.getFileName.toString }/> } }
    </chapter>
  }

  def renderSection(source: Path)(implicit sectionName: String): Elem = {
    ...
    val content = {
      <section xmlns="http://docbook.org/ns/docbook" xmlns:od="http://orc.csres.utexas.edu/OrcDocBook.xsd">
        { generateHeaderComment(Some(source.getFileName.toString)) }
