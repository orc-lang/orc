  ...
  def main(args: Array[String]): Unit = {
    ...
    for (n <- 0 to maxN) {
      ...
      out.append(s"""//
                    ...
                    |
                    |""".stripMargin)
      if (n > 0) out.append(s"""import scala.reflect.ClassTag
                               |
                               |""".stripMargin)
      out.append(s"""import orc.{ DirectInvoker, Invoker, OrcRuntime, SiteResponseSet, VirtualCallContext }
                    |import orc.error.runtime.{${if (n > 0) " ArgumentTypeMismatchException," else ""} ArityMismatchException, HaltException }
                    |
                    |""".stripMargin)
      if (n > 0) out.append(s"""import InvocationBehaviorUtilities.valueHasTypeByTag
                               |
                               |""".stripMargin)
