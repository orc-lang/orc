...
abstract class Site0Base extends HasGetInvoker0 with Site0 {
  ...
  protected def invoker[T <: Site0Base]
    ...
    new TargetClassAndArgumentClassSpecializedInvoker(exampleTarget, Array[Any]().asInstanceOf[Array[AnyRef]])
          with Site0Base.ImplInvoker[T] with MaybeInlinableInvoker {
      val impl = _impl
      final override val inlinable = Site0Base.this.inlinable
    }
  }

  protected def invokerInline[T <: Site0Base]
        (exampleTarget: T)
        (_impl: (VirtualCallContext, T) => SiteResponseSet): Invoker = {
    new TargetClassAndArgumentClassSpecializedInvoker(exampleTarget, Array[Any]().asInstanceOf[Array[AnyRef]])
          with Site0Base.ImplInvoker[T] with InlinableInvoker {
      val impl = _impl
    }
  }
}
...
abstract class PartialSite0Base extends HasGetDirectInvoker0 with PartialSite0 {
  ...
  protected def invoker[T <: PartialSite0Base]
    ...
    new TargetClassAndArgumentClassSpecializedInvoker(exampleTarget, Array[Any]().asInstanceOf[Array[AnyRef]])
          with PartialSite0Base.ImplInvoker[T] with MaybeInlinableInvoker {
      val impl = _impl
      final override val inlinable = PartialSite0Base.this.inlinable
    }
  }

  protected def invokerInline[T <: PartialSite0Base]
        (exampleTarget: T)
        (_impl: (T) => Option[Any]): DirectInvoker = {
    new TargetClassAndArgumentClassSpecializedInvoker(exampleTarget, Array[Any]().asInstanceOf[Array[AnyRef]])
          with PartialSite0Base.ImplInvoker[T] with InlinableInvoker {
      val impl = _impl
    }
  }
}
...
abstract class TotalSite0Base extends HasGetDirectInvoker0 with TotalSite0 {
  ...
  protected def invoker[T <: TotalSite0Base]
    ...
    new TargetClassAndArgumentClassSpecializedInvoker(exampleTarget, Array[Any]().asInstanceOf[Array[AnyRef]])
          with TotalSite0Base.ImplInvoker[T] with MaybeInlinableInvoker {
      val impl = _impl
      final override val inlinable = TotalSite0Base.this.inlinable
    }
  }

  protected def invokerInline[T <: TotalSite0Base]
        (exampleTarget: T)
        (_impl: (T) => Any): DirectInvoker = {
    new TargetClassAndArgumentClassSpecializedInvoker(exampleTarget, Array[Any]().asInstanceOf[Array[AnyRef]])
          with TotalSite0Base.ImplInvoker[T] with InlinableInvoker {
      val impl = _impl
    }
  }
}
