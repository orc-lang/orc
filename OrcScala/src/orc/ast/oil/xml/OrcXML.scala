...
// Project OrcScala
//
// $Id: OrcXML.scala 2773 2011-04-20 01:12:36Z jthywissen $
//
...
object OrcXML {
        ...
        <hole>
          <context>
            {for ((n, a) <- context) yield 
              <binding name={n}>{toXML(a)}</binding>
            }
          </context>
          <typecontext>
            {for ((n, t) <- context) yield 
              <binding name={n}>{toXML(t)}</binding>
            }
          </typecontext>
        </hole>
      case Top() => <top/>
  ...
  def anyToXML(a: Any): Elem = {
    a match {
      ...
      case _ => throw new Error("Could not serialize value " + a.toString + " to XML.")
    }
  }
    ...
    xml --> {
      ...
      case <hole><context>{ctx@ _*}</context><typecontext>{typectx@ _*}</typecontext></hole> => { 
        val context = HashMap.empty ++ { 
          for (b@ <binding>{a}</binding> <- ctx) yield 
            ((b \ "@name").text, argumentFromXML(a))
        }
        val typecontext = HashMap.empty ++ {
          for (b@ <binding>{t}</binding> <- typectx) yield 
            ((b \ "@name").text, typeFromXML(t))
        }
