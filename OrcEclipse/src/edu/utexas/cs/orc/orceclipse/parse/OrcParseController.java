...
/**
 * A parsing environment (file, project, etc.) that IMP constructs when it will
 * request Orc parsing.
 *
 * @author jthywiss
 */
...
//        }
    ...
    /**
     * Returns an Iterator that iterates over the tokens contained within the
     * given region, including any tokens that are only partially contained.
     * <p>
    ...
    /**
     * Parse the given source and return the resulting AST. The AST should is
     * cached, so that immediately after a successful parse,
     * {@link #getCurrentAst()} returns the same AST as this method produced.
     * <p>
     * {@link #initialize(IPath, ISourceProject, IMessageHandler)} must be
...
//        parser.resolve(currentAst);
    ...
    /**
     * @return the current parse string, as passed to the last invocation of
     *         {@link #parse(String, IProgressMonitor)}, or null
     */
    ...
    /**
     * @return the current AST, as created by the last invocation of
     *         {@link #parse(String, IProgressMonitor)}, or null
     */
    @Override
