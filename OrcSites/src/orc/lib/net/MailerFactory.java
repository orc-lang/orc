 ...
 * href="http://java.sun.com/products/javamail/javadocs/">JavaMail API</a>, with
 * a few key changes to simplify use in Orc:
 * <ul>
 * <li>{@link Mailer} wraps a Session object created from a properties file.
 * <li>{@link OrcMessage} provides a cooperatively-threaded wrapper for blocking
 * {@link Message} operations.
 ...
 * </ul>
 * <p>
 * This adds the following custom properties:
 * <ul>
 * <li>orc.mail.from.separator: separator used by
 * {@link Mailer#newFromAddress()}.
 * <li>orc.mail.password: password used by default.
 * <li>orc.mail.PROTOCOL.password: password used for a specific PROTOCOL (e.g.
 * smtp).
 * <li>orc.mail.inbox: name of inbox used by {@link OrcStore#getInbox()}.
 * <li>orc.mail.quota.duration: how long a quota period lasts
 * <li>orc.mail.quota.burst: maximum number of messages that can be sent during
 * any one quota period
 * </ul>
 *
 * @author quark
 */
public class MailerFactory extends SiteAdaptor {
    ...
    private static abstract class QuotaManager {
        /**
         * Call before sending a message, blocking if the quota does not allow a
         * send yet.
         */
     ...
     * {@link #duration} milliseconds. Think of mails as occupying slots in a
     * buffer of fixed size which is emptied regularly.
     * <p>
    ...
    private static class BurstQuotaManager extends QuotaManager {
        ...
        /*
         * FIXME: this implementation triggers a ValidationError when
         * Kilim-processed. public void use(int count) throws
         * InterruptedException { int durations; long currentTime =
         * System.currentTimeMillis(); if (currentTime - duration > endTime) {
         * // start a new burst endTime = currentTime; remaining = burst; } if
         * (count < remaining) { // no need to use up any new durations
         * remaining -= count; durations = 0; } else { // use up some new
         * durations count -= remaining; remaining = burst - count % burst;
         * durations = 1 + (count / burst); } // delay the end of the period
         * endTime += duration * durations; // wait until the end of the period
         * if (currentTime < endTime) { Task.getCurrentTask().wait(endTime -
         * currentTime); } }
         */
    }
    ...
    public static class Mailer {
        ...
        public Session getSession() {
            if (session == null) {
                session = Session.getInstance(properties, new Authenticator() {
                    /**
                     * Hack so that orc.mail.password property can be used to
                     * supply a password.
                     */
                    @Override
        ...
        /**
         * Generate a new random "FROM" address, of the following form: USER +
         * orc.mail.from.separator + unique id + "@" + HOST
         * 
    ...
    public static class OrcTransport {
         ...
         * this on messages returned from a store, only on those created by your
         * program (e.g. via {@link Mailer#newMessage()}).
         */
    ...
    public static class OrcStore {
        ...
        /**
         * Relies on the custom property orc.mail.inbox which should be set to
    ...
    /**
     * Wraps methods of {@link Message} to make them use cooperative threading
     * and return {@link OrcFolder} when appropriate. Also added utility methods
     * {@link #getText()} and {@link #delete()}.
     *
     * @author quark
     */
    public static class OrcMessage {
        ...
        public OrcMessage autoReply(final boolean replyToAll) throws MessagingException {
            ...
            final OrcMessage reply = new OrcMessage(message.reply(replyToAll));
        ...
        public boolean safeToAutoRespond() throws MessagingException {
            final String[] autoSubmittedHdrs = message.getHeader("Auto-Submitted");
            final String[] returnPathHdrs = message.getHeader("Return-Path");
            final Address[] replyTo = message.getReplyTo();
            final String[] precedenceHdrs = message.getHeader("Precedence");
            return
            ...
            // No Return-Path: <>
                    (returnPathHdrs.length == 0 || returnPathHdrs.length == 1 && !returnPathHdrs[0].equalsIgnoreCase("<>")) && replyTo != null && replyTo.length > 0 && !replyTo[0].toString().toUpperCase().contains("MAILER-DAEMON@") &&
